...
public class RedisUtil {
	...
	private static int redisCount = 0;
	/**
	 * redis过期时间,以秒为单位
	 */
	...
	private static void initialPool() {
		...
		jedisPool = new JedisPool(config, Constant.ADDR_ARRAY.split(",")[0],
				Constant.PORT, TIMEOUT);
	}
	...
	private static synchronized void poolInit() {
		if (jedisPool == null) {
			logger.info("jedispool init" + (++poolCount) + " 次数");
			logger.info("########");
			logger.info("");
			initialPool();
		}
	...
	public static void returnResource(final Jedis jedis) {
		if (jedis != null && jedisPool != null) {
			...
			// logger.info("return jedis:" + jedis);
		}
	}
}