...
public class POIUtil {
		...
		for (int i = 0, len = headInfoList.size(); i < len; i++) {
			...
			if (headInfo.containsKey("columnWidth")) {
				hssfSheet
						.setColumnWidth(
								i,
								(short) (((Integer) headInfo.get("columnWidth") * 8) / ((double) 1 / 20)));
			}
		}
	}
	 ...
	 * @param dataList
	 */
	public void writeContent(HSSFWorkbook hssfWorkbook, HSSFSheet hssfSheet,
			int startIndex, List<Map<String, Object>> headInfoList,
			List<Map<String, Object>> dataList) {
		Map<String, Object> headInfo = null;
		HSSFCellStyle cs = hssfWorkbook.createCellStyle();
		cs.setAlignment(CellStyle.ALIGN_CENTER);// 垂直对齐居中  
		HSSFRow r = null;
		HSSFCell c = null;
		// 处理数据
		Map<String, Object> dataItem = null;
		Object v = null;
		for (int i = 0, rownum = startIndex, len = (startIndex + dataList
				.size()); rownum < len; i++, rownum++) {
			...
			for (int j = 0, jlen = headInfoList.size(); j < jlen; j++) {
				...
				if (v instanceof String) {
					...
					c.setCellValue((Double) v);
						...
						|| v instanceof Short || v instanceof Float) {
	 ...
	 * POIUtil.exportExcel2FilePath("test sheet 1","F:\\temp\\customer2.xls",
