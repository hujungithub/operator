...
<body >
  <div class="" style="background-color:white!important; border-radius:5px; padding:10px;">
	  ...
	  <div class="home-label home-label-font margin-right add-wrap">
         <button type="button" id="add-btn" class="je-btn je-bg-orange je-rdu">新建用户</button>
  ...
  var data =  getDataFromServer();
       table = $('#example').DataTable({
          ...
          "columns":[
              ...
              '<a class="delete-btn">删除</a>'+'&nbsp;'+'<a type="button" class="role-btn">角色分配</a>'} */
          ],
          "columnDefs": [{
              "searchable": false,
              "orderable": false,
              ...
              "render": function(data, type, row) {
                  ...
                  "<button id=\"delete-btn\" class=\"je-btn je-btn-mini je-bg-red je-f12\" style=\"margin-top:4px;\" onclick='delete("+data+");'>删除</button>&nbsp;"+"<button id=\"role-btn\" class=\"je-btn je-btn-mini je-bg-olive je-f12\" style=\"margin-top:4px;\" onclick='giverole("+data+");'>角色分配</button>";
              },      
              "targets": 7
          }],
          "bServerSide":false,
...
$("#add-btn").on("click",function(){
	jeBox.open({
         ...
         closefun: function(index, id){
		    window.location.href='jsp/SM/userManager.jsp?operatorloginid='+operatorloginid;
		 },
		 endfun :function(index, id){
		    window.location.href='jsp/SM/userManager.jsp?operatorloginid='+operatorloginid;
		 },
         content: '<%=basePath%>jsp/SM/userOperation.jsp?operatorloginid='+operatorloginid
    });
});
...
//保存
  ...
  //批量删除
    ...
    $(".user-detail-btn").click(function() {
      ...
      var data=row.data();
      location.href = "jsp/SM/detail_pile.html?obj="+escape(JSON.stringify(data));
    });
...
function sendDataToserver(data){
  ...
  $.ajax({
      ...
      "url":"system/deleteSysUserByIds",//后台接口
      "data":{
          id:id,
      },
      "type":"post",
...
function update(id){
    jeBox.open({
      ...
      content: '<%=basePath%>jsp/SM/userOperation.jsp?id='+id+'&operatorloginid='+operatorloginid
  });
};
...
function giverole(id){
    jeBox.open({
      ...
      maxBtn: true,
      scrollbar: false,
      content: '<%=basePath%>jsp/SM/distributeRole.jsp?id='+id                
   });
};
