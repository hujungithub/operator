...
<div class="je-p20">
		<form action="" id="add_form">
            ...
            <div class="je-form-item">
                <div class="je-w33 je-dib">
                 ...
                 <!-- <div class="je-w33 je-dib">
                    <label class="je-label je-f14">厂商ID:</label>
                    <div class="je-inputbox" style="">
                        <input type="text" name="MFRID" id="MFRID" class="my-input je-input">
                    </div>
                </div> -->
            </div>
			<input id="OPERATORLOGINID" name="OPERATORLOGINID" type="hidden" value="${param.operatorloginid}">
		    <input id="MFRID" name="MFRID" type="hidden" value="<%=request.getParameter("MFRID")%>">
			<input id="oldMODEL" name="oldMODEL" type="hidden" value="<%=request.getParameter("MODEL")%>">
		</form>
	...
	$(document).ready(function(){
	    var MFRID = $("#MFRID").val();
		var MODEL = $("#oldMODEL").val(); 
		$.ajax({
			"type":"post",
			"url":"manufacturers/findUpdateManuf",
			"data":{
	...
	function updateOMC(data) {
		 var str;
		 $("#MFRID").val(data.mfrid);
		 $("#MFRNAME").val(data.mfrname);
		 $("#MFRABBR").val(data.mfrabbr);  
		 $("#MODEL").val(data.model);
		 $("#RATEDPOWER").val(data.ratedpower);
	     $("#CPTYPE").val(data.cptype);
		 $("#CPPHASE").val(data.cpphase);
		 /* $("#MODEL2").val(); */
		 $("#INTERFACECOUNT").val(data.interfacecount);
	 }
