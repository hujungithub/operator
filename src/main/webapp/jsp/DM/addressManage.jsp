...
<body >
  <div class="" style="background-color:white!important; border-radius:5px; padding:10px;">
      <div class="h2 bgColor">
        <a style="text-decoration:underline">充电桩地址列表</a>
      </div>
      
      <div class="searchbox h2">
      <form action="" id="search-form" method="post">
      			<div>
      			<label><span style="width:100px">地址:</span></label>
      				<input data-column="4" id="search4" name="ADDRESSNAME" type="text" class="search"/>
      			</div>
		...
		<div class="home-label home-label-font margin-right add-wrap">
			<shiro:hasPermission name="chargeaddress:add">
	          <button type="button" id="add-btn" class="je-btn je-bg-orange je-rdu">新建地址</button>
	        </shiro:hasPermission>
	        <shiro:hasPermission name="chargeaddress:delete">
	          <button type="button" id="del-btn" class="je-btn je-bg-red je-rdu">批量删除</button>
			</shiro:hasPermission>
	        <shiro:hasPermission name="chargeaddress:import">  
  ...
  //批量删除
    ...
    //点击增加
         $("#add-btn").on("click",function(){
              ...
              jeBox.open({
                ...
                closefun: function(index, id){
        		    window.location.href='jsp/DM/addressManage.jsp?operatorloginid='+operatorloginid;
        		 },
        		 endfun :function(index, id){
        		    window.location.href='jsp/DM/addressManage.jsp?operatorloginid='+operatorloginid;
        		 },
                content: ['<%=basePath%>address/addressManageAddFindData?operatorloginid=' + operatorloginid,'yes']
            });
        }) 
       ...
       $("#export-btn").on("click",function(){
    	   var operatorloginid = $("#OPERATORLOGINID").val();
    	   $("#search-form").attr("action","address/addressExport");
		   $("#search-form").submit();
       });
      ...
      table = $('#example').DataTable({
         ...
         "columnDefs": [{
             ...
             "render": function(data, type, row) {
                 return "<shiro:hasPermission name='chargeaddress:update'><button id=\"update-btn\" class=\"je-btn je-btn-mini je-f12\" style=\"margin-top:4px;\" onclick='update("+data+");'>编辑</button>&nbsp;</shiro:hasPermission>"+
                 "<shiro:hasPermission name='chargeaddress:delete'><button id=\"delete-btn\" class=\"je-btn je-btn-mini je-bg-red je-f12\" style=\"margin-top:4px;\" onclick='delete("+data+");'>删除</button></shiro:hasPermission>";
             },      
             "targets": 7
         }],
...
function getDataFromServer(){
  ...
  var er = jeBox.loading(1,"玩命加载中");
  $.ajax({
      "url":"address/findAddressFirst",//后台接口
      "data":{
    	  "operatorloginid":operatorloginid
      },
      "type":"GET",
  ...
  return data;
    ...
    function update(id){
      ...
      jeBox.open({
        ...
        content: ['<%=basePath%>jsp/DM/addressUpdate.jsp?operatorloginid=' 
      		  + operatorloginid + '&ADDRESSID=' + id,'yes']
     	});
    };
  </script>
</html>