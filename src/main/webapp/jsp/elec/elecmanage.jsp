      ...
      <div class="h2 bgColor" style="color:white">
		   ...
		   <%--  <shiro:hasPermission name="chargemanufacturer:add">
		    ...
		    <shiro:hasPermission name="chargemanufacturer:addmodel">
		          <button type="button" id="addelec-btn" class="je-btn je-bg-orange je-rdu" style="margin-top:15px">增加电工信息</button>
			</shiro:hasPermission>
			<shiro:hasPermission name="chargepile:delete">
		          	<button type="button" id="del-btn" class="je-btn je-bg-red je-rdu" style="margin-top:15px">批量删除</button>
			</shiro:hasPermission>
			</div>
      </div>
      <div class="searchbox">
      	<form action="" method="post" id="search-form">
      		...
      		<div style="float:left">
		      	<span class="h2" style="width:50px ">电工编号:</span>
				<input data-column="1" name="ELECID" class="search" id="col1_filter" style="margin-top: 14px;width:22%;height:25px" type="text"  />
		        <span class="h2" style="width:50px">姓名：</span>
		        <input data-column="2" name="ELECNAME" class="search" id="col2_filter" style="margin-top: 14px;width:22%;height:25px"" type="text"  />
      		</div>
...
<script type="text/javascript">
       ...
       table = $('#example').DataTable({
          ...
          "columnDefs": [{
              ...
              "render": function(data, type, row) {
                  return "<shiro:hasPermission name='chargemanufacturer:update'><button id=\"update-btn\" class=\"je-btn je-btn-mini je-f12\" style=\"margin-top:4px;\" onclick='update("+row.elecid+")'>编辑</button>&nbsp;</shiro:hasPermission>"+
                  ...
                  "<shiro:hasPermission name='chargemanufacturer:delete'><button id=\"check-btn\" class=\"je-btn je-btn-mini je-bg-grey je-f12\" style=\"margin-top:4px;\" onclick='checkElecInfo("+row.elecid+")'>审核</button></shiro:hasPermission>";
              },      
              "targets": 9
          }],
          "createdRow": function ( row, data, index ) {
              $('td', row).eq(2).css('color','#FF0066');
              if (data.status == '未审核') {
            	  $('td', row).eq(7).css('color','#FF0066');
              }else if(data.status == '已审核'){
            	  $('td', row).eq(7).css('color','#339900');
              }
          },
          "bServerSide":false,
          "scrollX": true,
 ...
 $("#example tbody").on("click","#delete-btn",function(){
    ...
    $.each(tds, function(i,val){
      var jqob=$(val);
      if(i > 8 || i < 1){return true;}//跳过第1项 序号,按钮
      var txt = jqob.text();
      jqob.html(txt);
...
function sendDataToserver(data){
	...
	for(var i=0;i<data.length;i++){
		elecid+=","+data[i].elecid;
	  }
  $.ajax({
      "url":"elec/deleteElec",//后台接口,删除型号
      "data":{
    	  "elecid":elecid
      },
      "type":"GET",
...
/* function sendDataToCheck(data){
  ...
  $.ajax({
      "url":"elec/checkElecInfo",//后台接口,删除型号
      "data":{
    	  "elecid":elecid
      },
      "type":"GET",
...
function update(elecid){
    ...
    jeBox.open({
	 ...
	 endfun :function(index, id){
      ...
      content: ['<%=basePath%>jsp/elec/elecUpdate.jsp?elecid=' 
    		  + elecid,'yes']
   	});
  };
 ...
 // 审核
function checkElecInfo(elecid){
	 var operatorloginid = $("#OPERATORLOGINID").val();
	    $.ajax({
		      "url":"elec/updateElecInfo",//后台接口
		      "data":{
		    	  "elecid":elecid
		      },
		      "type":"GET",
