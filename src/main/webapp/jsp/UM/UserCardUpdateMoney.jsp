...
<div class="je-p20">
		<form action="" id="add_form">
            <div class="je-form-item">
                ...
                <div class="je-w33 je-dib">
                    <label class="je-label je-f14">当前余额:</label>
                    <div class="je-inputbox" style="">
	                        <input id="ACOUNTMONEY" name="ACOUNTMONEY" class="je-input"
	                        value="<%=request.getParameter("ACOUNTMONEY")%>" readonly="readonly"/>
                    </div>
                </div>
            </div>
             <div class="je-form-item">
                <div class="je-w33 je-dib">
                    <label class="je-label je-f14">修改余额为:</label>
                    <div class="je-inputbox" style="">
	                        <input id="MODIFIEDACCOUNTSUM" name="MODIFIEDACCOUNTSUM" class="je-input" value=""/>
                    </div>
                </div>
            </div>
	...
	$(document).ready(function(){
		...
		$.ajax({
			"type":"post",
			"url":"userCard/findUpdateBalance",
			"data":{
				"CARDNUM":CARDNUM
			},
		 	"dataType":"json", 
		...
		$("#add_form").validate({
			...
			rules: {
				MODIFIEDACCOUNTSUM: "required"				
			},
			messages:{
				MODIFIEDACCOUNTSUM: "运营商名称不能为空！"		
			},
		});
	});
	...
	function updateDATA(data) {
		...
		$("#ACOUNTMONEY").val(data.accountsum);
	};
