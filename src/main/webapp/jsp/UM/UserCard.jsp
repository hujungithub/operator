...
<body >
  <div class="" style="background-color:white!important; border-radius:5px; padding:10px;">
      ...
      <form action="" id="search-form">
      			...
      			<span>卡用户姓名：</span> 
      				<input data-column="2" id="search2" name="CARDUSERNAME" type="text" class="search" />
      			<span>手机号:</span> 
      				<input data-column="4" id="search4" name="TELEPHONE" type="text" class="search"/>
      			<span>运营商:</span> 
      				<input data-column="12" id="search12" name="OPERATORNAME" type="text" class="search"/>
      			</div>
				<span>开卡日期:</span> 
				...
				<div class="home-label home-label-font margin-right add-wrap">
					<shiro:hasPermission name="usercard:moneyupdate">
					  <button type="button" id="updateMoney-btn" disabled="disabled"
					 				class="je-btn je-bg-orange je-rdu">修改余额</button>
					</shiro:hasPermission>
					<shiro:hasPermission name="usercard:import">
...
$.fn.dataTable.ext.search.push(
    function( settings, data, dataIndex ) {
        ...
        var date = data[10]; // use data for the age column
        if ( 
             (inpstart=="" && inpend=="") ||
...
$(document).ready(function() {
 		initJsp();
 		$("#updateMoney-btn").attr("disabled",true);
...
//删除
  ...
  //批量选择
    $('#example tbody').on( 'click', 'tr', function () {
      ...
      if(length == 1){
    	  $("#updateMoney-btn").attr("disabled",false);
      }else{
    	  $("#updateMoney-btn").attr("disabled",true);
      }
    });
  ...
  //批量删除
    ...
    $('#example tbody').on('click', 'td.details-control', function () {
      ...
      //修改余额
      	$("#updateMoney-btn").on("click",function(){
      		...
      		var cardnum = dataArr[0].cardnum;
      		var acountMoney = dataArr[0].accountsum;
	   		 jeBox.open({
	   	          title:"修改余额",
	   	          type: 'iframe',
	   	        	boxSize: ['70%', '40%'],
	   	          ...
	   	          closefun: function(index, id){
	   			    window.location.href='jsp/UM/UserCard.jsp?operatorloginid='+operatorloginid;
	   			 },
	   			 endfun :function(index, id){
	   			    window.location.href='jsp/UM/UserCard.jsp?operatorloginid='+operatorloginid;
	   			 },
	   	          content: ['<%=basePath%>jsp/UM/UserCardUpdateMoney.jsp?CARDNUM=' 
	   	        		  + cardnum + '&ACOUNTMONEY=' + acountMoney,'yes']
	   	       	});
      	});
       ...
       $("#export-btn").on("click",function(){
    	   var operatorloginid = $("#OPERATORLOGINID").val();
    	   $("#search-form").attr("action","userCard/userCardExport");
		   $("#search-form").submit();
       });
      ...
      table = $('#example').DataTable({
         ...
         "columnDefs": [{
             ...
             "render": function(data, type, row) {
                 return "<shiro:hasPermission name='usercard:update'><button id=\"update-btn\" class=\"je-btn je-btn-mini je-f12\" style=\"margin-top:4px;\" onclick='update("+data+");'>编辑</button>&nbsp;</shiro:hasPermission>"
                 /* +"<button id=\"delete-btn\" class=\"je-btn je-btn-mini je-bg-olive je-f12\" style=\"margin-top:4px;\" onclick='delete("+data+");'>详情</button>" */;
             },      
             "targets": 13
         }],
...
function getDataFromServer(){
  ...
  var er = jeBox.loading(1,"玩命加载中");
  $.ajax({
      "url":"userCard/findUserCardFirst",//后台接口
      "data":{
    	  "operatorloginid":operatorloginid
      },
      "type":"GET",
  ...
  return data;
    ...
    function update(id){
       ...
       jeBox.open({
         ...
         content: ['<%=basePath%>jsp/UM/UserCardUpdate.jsp?operatorloginid=' 
       		  + operatorloginid + '&CARDNUM=' + id,'yes']
      	});
     };
  </script>
</html>