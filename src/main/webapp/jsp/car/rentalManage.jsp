  ...
  <div class="" style="background-color:white!important; border-radius:5px; padding:10px;">
      ...
      <form action="" id="search-form">
      			...
      			<label><span style="width:100px">地址:</span></label>
      				<input data-column="8" id="search8" name="LOCATION" type="text" class="search"/>
      			<span>桩ID：</span> 
      				<input data-column="1" id="search1" name="CPID" type="text" class="search" />
      			<span>运营商:</span> 
      				<input data-column="4" id="search4" name="OPERATORID" type="text" class="search"/>
      			<span>充电站:</span> 
      				<input data-column="3" id="search3" name="CSNAME" type="text" class="search"/>
      			<span>费率:</span> 
      				<select  onchange="xixi()">
      					<option value="">请选择</option>
      					<option value="1">1</option>
      					<option value="2">2</option>
      				</select>
      				<input  id="rate" type="text" class="search" value=""/>
      			</div>
				<span>建桩日期:</span> 
				...
				<div class="home-label home-label-font margin-right add-wrap">
		          <button type="button" id="add-btn" class="je-btn je-bg-orange je-rdu">新建充电桩</button>
...
function xixi(){
	/* var kk=osel.options[osel.selectedIndex].text;
	alert(kk); */
	
	$("#rate").attr("data-column","3");
	$("#rate").attr("id","search3");
	$("#rate").val("1");
}
      ...
      table = $('#example').DataTable({
         ...
         },
         {
             "render": function(data, type, row, meta) {
                 ...
                 return '<a href="jsp/DM/chargePileDetail.jsp?CPID=' + row.cpid + '" style="color:#0095d9">' + row.cpname + '</a>';
             },
             //指定是第三列
             "targets": 2
         }],
...
function getDataFromServer(){
  ...
  var er = jeBox.loading(1,"玩命加载中");
  $.ajax({
      "url":"chargePile/findCPFirst",//后台接口
      "data":{
    	  "operatorloginid":operatorloginid
      },
      "type":"GET",
