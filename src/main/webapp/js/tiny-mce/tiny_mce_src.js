	...
	tinymce.Formatter = function(ed) {
		...
		function mergeSiblings(prev, next) {
			...
			function compareElements(node1, node2) {
				...
				if (!compare&quot;(getAttribs(node1), getAttribs(node2)))
					return FALSE;

				// Styles are not the same
				if (!compare&quot;(dom.parseStyle(dom.getAttrib(node1, 'style')), dom.parseStyle(dom.getAttrib(node2, 'style'))))
					return FALSE;
